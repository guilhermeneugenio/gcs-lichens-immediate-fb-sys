...
const LichensImagePicker = props => {
  ...
  let content = (index) => {
    if(options[index]!= 0){
      return(
      <View style={{...LichensImagePickerStylesheet.imageSelect, 
...
<View>
    <Modal visible={modalVisible} animationType="slide" transparent={true}>
      <View style={LichensImagePickerStylesheet.centeredView}>
        <View style={LichensImagePickerStylesheet.modalView}>
          ...
          />
          <View style={{...LichensImagePickerStylesheet.content, alignItems: "center", width: Dimensions.get('window').width * 0.7}}>
            <TouchableHighlight
    ...
    <View style={LichensImagePickerStylesheet.content} >
      {props.props.choices.map((item, index) => {
        return (
        <View key={index}>
          <TouchableHighlight onPress={onChange.bind(this, index)} underlayColor="green">
            <View>
              <Image style={{width: ((Dimensions.get('window').width*0.94-props.props.margin*props.props.imagesPerLine*2)/props.props.imagesPerLine) || (Dimensions.get('window').width*0.15),
              height: ((Dimensions.get('window').width*0.94-props.props.margin*props.props.imagesPerLine*2)/props.props.imagesPerLine) || (Dimensions.get('window').width*0.15),
              margin:  props.props.margin || Dimensions.get('window').width*0.01  }} 
